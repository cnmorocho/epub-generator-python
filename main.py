...
class Epub:
    ...
    def build(self):
        ...
        epub_file_path = os.path.join(self.output_dir, f'{self.metadata.title}.epub')
    ...
    def __create_toc_file(self):
        ...
        ET.SubElement(ET.SubElement(root, 'docTitle'), 'text').text = self.metadata.title
    ...
    def __create_chapters_files(self):
        for index, chapter in enumerate(self.content.chapters):
            ...
            tree.write(chapter.relative_path, encoding='utf-8', xml_declaration=True, method='xml')
    ...
    def __create_content_file(self):
        ...
        ET.SubElement(metadata, 'dc:creator', { 'opf:file-as': self.metadata.author, 'opf:role': 'aut' }).text = self.metadata.author
        ET.SubElement(metadata, 'dc:language').text = self.metadata.language
        if self.metadata.subjects:
            ET.SubElement(metadata, 'dc:subject').text = ', '.join(self.metadata.subjects)
        ET.SubElement(metadata, 'dc:description').text = self.metadata.description
        if self.metadata.publication_date:
            ET.SubElement(metadata, 'dc:date', { 'opf:event': 'publication' }).text = self.metadata.publication_date
